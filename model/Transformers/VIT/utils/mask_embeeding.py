...
def ShuffleIndex(index: list, sample_ratio: float):
    ...
    if len(index) < 4:
        ...
        remain_length = int((1 - sample_ratio) * len(index)) + 1
...
def MaskEmbeedingFix(token_emb, mask_index_list=None, cls_index=True):
    if mask_index_list is not None:
        token_length = token_emb.shape[1]
        token_index = [x for x in range(token_length)]
        for mi in mask_index_list:
            if cls_index:
                token_index.remove(mi+1)
            else:
                token_index.remove(mi)
        #token_index = [0] + token_index
        token_emb = token_emb[:, token_index, :]
    return token_emb
...
class UnMaskEmbeeding(nn.Module):
    ...
    def __init__(self, input_size, embed_dim, in_chans, patch_size, num_patches):
        ...
        self.raw_inputs = torch.ones((self.in_chans, input_size[0], input_size[1]))*127. / 255 
